...
export function AccountModal({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!account && formData.initial_balance) {
      const initialBalance = parseFloat(formData.initial_balance.replace(/\./g, '').replace(',', '.'));
      submitData.initial_balance = formData.type === 'credit' ? -Math.abs(initialBalance) : initialBalance;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {/* Saldo Inicial - apenas para contas novas */}
          {!account && (
            <div className="space-y-6 p-6 border rounded-lg bg-muted/20">
              <div className="space-y-2">
                <h3 className="text-lg font-medium">Saldo Inicial (Opcional)</h3>
                <p className="text-sm text-muted-foreground">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  <Label htmlFor="initial_balance" className="text-base font-medium">
                    {formData.type === 'on_budget' ? 'Saldo Atual' : 'Valor da Fatura Atual'}
                  </Label>
                  <CurrencyInput
                    id="initial_balance"
