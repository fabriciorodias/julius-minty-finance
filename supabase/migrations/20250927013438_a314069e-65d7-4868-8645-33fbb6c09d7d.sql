-- Corrigir função get_recurring_transactions_analytics para evitar vazamento de dados entre lançamentos recorrentes
...
AS $function$
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE t.user_id = rt.user_id
      AND t.counterparty_id = rt.counterparty_id
      AND t.category_id = rt.category_id
      AND t.type = rt.type
      AND rt.counterparty_id IS NOT NULL
