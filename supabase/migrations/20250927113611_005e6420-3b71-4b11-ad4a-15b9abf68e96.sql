-- Create a version of get_account_balances that accepts user_id as parameter
-- This bypasses the auth.uid() issue when called via RPC
CREATE OR REPLACE FUNCTION public.get_account_balances_for_user(
  p_user_id uuid, 
  p_as_of_date date DEFAULT (now())::date
)
